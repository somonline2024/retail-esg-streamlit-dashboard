...
- Added "🔄 Refresh Data" buttons for manual refresh
...
def make_new_item(form: Dict) -> Dict:
    ...
    return {
        ...
        'daily_sales': ','.join(
            [str(int(x)) if float(x).is_integer() else str(x) for x in ds_array]),
...
with col1:
    ...
    uploaded_file = st.file_uploader(
        'Upload CSV (columns: name,daily_sales,shelf_life_days,base_price,stock,demand_score)',
        type=['csv'])
    if uploaded_file is not None:
        try:
            ...
            st.button("🔄 Refresh Data", key="refresh_csv")
    ...
    with st.form('add_item_form'):
        ...
        demand_score = st.slider('Demand score (0-1)',
        ...
        if submitted:
            ...
            if new_item['name']:
                ...
                st.button("🔄 Refresh Data", key="refresh_add")
            else:
    ...
    if inv.empty:
        ...
        df_demand = inv[['name', 'demand_pct']].sort_values(
            'demand_pct', ascending=False).head(10)
        fig_demand = px.bar(df_demand, x='name', y='demand_pct',
                            title='Demand score (%)')
        st.plotly_chart(fig_demand, use_container_width=True)

        df_pie = df_stock.copy()
        fig_pie = px.pie(df_pie, names='name', values='stock',
                         title='Stock distribution (top 10)')
        ...
        if user_msg and user_msg.strip():
            ...
            try:
                with st.spinner('Contacting assistant...'):
                    ...
                    r.raise_for_status()
                    data = r.json()
                    assistant_reply = data.get('assistant_reply') or data.get(
                        'natural_response') or 'Response received'
                    st.session_state.chat_messages.append(
                        {'role': 'assistant', 'content': assistant_reply, 'data': data.get('data')})
    ...
    for msg in st.session_state.chat_messages[-12:][::-1]:
        if msg['role'] == 'user':
            ...
            if msg.get('error'):
                ...
                if msg.get('data'):
                    if st.checkbox('Show raw data for latest assistant reply',
                                   key=f"raw_{len(st.session_state.chat_messages)}"):
                        st.json(msg['data'])

    st.markdown('---')
    st.header('Image Detection')
    uploaded_image = st.file_uploader('Upload an image for detection',
                                      type=['png', 'jpg', 'jpeg'])
                ...
                with st.spinner('Analyzing...'):
                    ...
                    if d.get('items_detected'):
                        ...
                        for it in d.get('items_detected'):
                            st.write(
                                f"- {it.get('item')} — confidence: {it.get('confidence')}")
            except Exception as e:
...
if not st.session_state.inventory:
    ...
    show_df = inv_df[['id', 'name', 'stock', 'base_price',
                      'shelf_life_days', 'demand_score', 'daily_sales']]
    ...
    if st.button('Delete item'):
        if to_delete:
            st.session_state.inventory = [
                it for it in st.session_state.inventory if it['name'] != to_delete]
            st.success(f"Deleted {to_delete}")
            st.button("🔄 Refresh Data", key="refresh_delete")
        else:
